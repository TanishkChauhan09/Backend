...
<!-- jo code me js me likh skta whi same code m typescript me bhi likh skta hoon as T.S is a superset of J.S-->
...
1: Partial<Person> -> Partial se properties become optional
2: Required<Person> -> Required se all properties should be filled
3: Readonly<Person> -> property can only be read not changed after assigning value to it

<!-- Array of Objects -->
interface people{name:string,age:number};
let p1:people[] = [{"rohan",25},{"Sohan",2}]
...
placeorder(80,(amount)=>{
    console.log(amount);
})

<!-- // Rest parameter -->

function heet(...arr:number[]){
    let ans:number=0;
    arr.forEach((val:number)=> ans = ans+val);
    console.log(ans);

}
heet(3,4,6,5,2,3,6,8,78,987,87);
...
<!-- Default parameter -->
function meet(name:string="Rohan"){
    console.log(name);
}
meet(); // parameter pass nhi kiya toh dafault wala hi chla jayega aur Rohan print hojayega
meet("Sohan"); // print sohan hoga
...
class person{
    ...
    // {
    //     this.name = name; (this.name me name wala person wala name hai, aur equal to ke baad wala name me jo m pass kr rha hoon wo iss name me aakr person wale name me store horha hai)
    //     this.age = age; (this.age me age wala person wala age hai, aur equal to ke baad wala age me jo m pass kr rha hoon wo iss age me aakr person wale age me store horha hai)
    // }
